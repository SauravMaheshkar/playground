...
fn is_subtree(x: Option<Tree>, y: Option<Tree>) -> bool {
    if x.is_none() || y.is_none() {
        return false;
    }
    if check_same(x.clone(), y.clone()) {
        return true;
    }

    return check_same(x.as_ref().unwrap().borrow().left.clone(), y.clone())
        || check_same(x.as_ref().unwrap().borrow().right.clone(), y.clone());
}
...
pub fn main(test_cases: Vec<TreeTreeBoolTestCase>) {
    for (_, test_case) in test_cases.iter().enumerate() {
        ...
        let output = is_subtree(x_tree, y_tree);
